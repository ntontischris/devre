'use client';

import { useState } from 'react';
import { Menu } from 'lucide-react';
import { useTranslations } from 'next-intl';
import { Button } from '@/components/ui/button';
import { SalesmanBreadcrumbs } from './breadcrumbs';
import { ThemeToggle } from '@/components/admin/theme-toggle';
import { NotificationBell } from '@/components/admin/notification-bell';
import { SalesmanUserNav } from './user-nav';
import { SalesmanMobileNav } from './mobile-nav';
import { LanguageSwitcher } from '@/components/shared/language-switcher';

export function SalesmanHeader() {
  const t = useTranslations('common');
  const [mobileNavOpen, setMobileNavOpen] = useState(false);

  return (
    <>
      <header className="sticky top-0 z-10 flex h-16 items-center gap-4 border-b border-stone-200 bg-stone-50/80 backdrop-blur-sm dark:bg-zinc-900/80 dark:border-zinc-800 px-4 md:px-6">
        {/* Mobile menu trigger */}
        <Button
          variant="ghost"
          size="icon"
          className="md:hidden"
          onClick={() => setMobileNavOpen(true)}
          aria-label={t('openMenu')}
        >
          <Menu className="h-5 w-5" />
        </Button>

        {/* Breadcrumbs */}
        <div className="flex-1">
          <SalesmanBreadcrumbs />
        </div>

        {/* Right side actions */}
        <div className="flex items-center gap-2">
          <LanguageSwitcher />
          <ThemeToggle />
          <NotificationBell />
          <SalesmanUserNav />
        </div>
      </header>

      {/* Mobile navigation */}
      <SalesmanMobileNav open={mobileNavOpen} onOpenChange={setMobileNavOpen} />
    </>
  );
}
