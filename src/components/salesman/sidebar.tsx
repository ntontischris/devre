...
export function SalesmanSidebar() {
  ...
  const NAV_ITEMS = [
    ...
    {
      href: '/salesman/resources',
      icon: FolderOpen,
      label: t('resources'),
    },
    {
      href: '/salesman/handbook',
      icon: BookOpen,
      label: t('handbook'),
    },
    {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('salesman-sidebar-collapsed');
    if (stored !== null) {
  ...
  const toggleCollapse = () => {
    setIsCollapsed((prev) => {
      const newValue = !prev;
      localStorage.setItem('salesman-sidebar-collapsed', String(newValue));
      return newValue;
    });
  };
