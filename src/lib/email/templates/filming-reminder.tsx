import { Text, Button, Section } from '@react-email/components';

import { BaseLayout } from './base-layout';
import { getEmailTranslations } from '../translations';

interface FilmingReminderEmailProps {
  clientName: string;
  packageName: string;
  packageDeliverables: string;
  locale: 'el' | 'en';
  ctaUrl: string;
}

export function FilmingReminderEmail({
  clientName,
  packageName,
  packageDeliverables,
  locale,
  ctaUrl,
}: FilmingReminderEmailProps) {
  const t = getEmailTranslations(locale).filmingReminder;

  return (
    <BaseLayout preview={t.subject} locale={locale}>
      <Text style={greeting}>{t.greeting(clientName)}</Text>
      <Text style={bodyText}>{t.body(packageName, packageDeliverables)}</Text>

      <Section style={ctaSection}>
        <Button style={ctaButton} href={ctaUrl}>
          {t.cta}
        </Button>
      </Section>

      <Text style={footerNote}>{t.footer}</Text>
    </BaseLayout>
  );
}

const greeting = {
  fontSize: '16px',
  color: '#18181b',
  marginBottom: '16px',
} as const;

const bodyText = {
  fontSize: '15px',
  color: '#3f3f46',
  lineHeight: '1.6',
  marginBottom: '24px',
} as const;

const ctaSection = {
  textAlign: 'center' as const,
  marginBottom: '24px',
} as const;

const ctaButton = {
  backgroundColor: '#7c3aed',
  color: '#ffffff',
  padding: '12px 32px',
  borderRadius: '6px',
  fontSize: '15px',
  fontWeight: '600' as const,
  textDecoration: 'none',
  display: 'inline-block',
} as const;

const footerNote = {
  fontSize: '13px',
  color: '#71717a',
  fontStyle: 'italic',
} as const;
